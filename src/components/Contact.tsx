  ...
  return (
    <section id="contact" className="relative z-[1] mx-auto max-w-[800px] px-[6vw] pb-[140px] pt-[100px] text-center">
      <motion.p
        ...
        viewport={{ once: true }}
        className="mb-2.5 font-mono text-[13px] tracking-[0.08em] text-accent-light"
      >
        05 — CONTACT
      </motion.p>
      <motion.h2
        ...
        viewport={{ once: true }}
        className="mb-10 inline-block font-display text-[clamp(32px,5vw,50px)] font-bold text-[#f5f5f9]"
      >
        Have something in mind? <span className="gradient-text">Let&apos;s talk.</span>
      </motion.h2>
      ...
      >
        <MagneticLink
          ...
          className="mb-8 inline-block rounded-2xl bg-gradient-to-r from-accent to-accent-light px-[38px] py-[18px] font-display text-lg font-semibold text-white no-underline shadow-[0_10px_30px_rgba(108,92,231,0.35)]"
        >
          {profile.email}
        </MagneticLink>
      </motion.div>
      ...
      <motion.div
        ...
        className="flex flex-wrap justify-center gap-7 font-mono text-[13.5px]"
      >
        <a
          href={profile.github}
          target="_blank"
          rel="noreferrer"
          className="flex items-center gap-[7px] text-paper/60 no-underline transition-colors hover:text-accent-light"
        >
      ...
      <p className="mt-[70px] font-mono text-xs text-paper/30">designed &amp; built by {profile.name}</p>
    </section>
  )
}
